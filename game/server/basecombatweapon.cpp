...
void CBaseCombatWeapon::OnPhysGunPickup(CBasePlayer *pPhysGunUser, PhysGunPickup_t reason)
{
...
//			reason - 
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::OnPhysGunDrop(CBasePlayer *pPhysGunUser, PhysGunDrop_t reason)
{
}

void CBaseCombatWeapon::Event_Killed(const CTakeDamageInfo &info)
{
	for (int i = 0; i < 8; i++)
	{
		ThrowChunks(this, info.GetDamagePosition(), RandomAngle(0, 360), g_PropDataSystem.GetRandomChunkModel("MetalChunks"));
	}
	BaseClass::Event_Killed(info);
}

void CBaseCombatWeapon::ThrowChunks(CBaseEntity *pWeapon, const Vector &vecChunkPos, const QAngle &vecChunkAngles, const char *pszChunkName)
{
	CGib *pChunk = CREATE_ENTITY(CGib, "gib");
	pChunk->Spawn(pszChunkName, 30.0f);
	pChunk->SetBloodColor(DONT_BLEED);

	pChunk->SetAbsOrigin(vecChunkPos);
	pChunk->SetAbsAngles(vecChunkAngles);

	pChunk->SetOwnerEntity(pWeapon);

	pChunk->SetSolidFlags(FSOLID_TRIGGER);
	...
	QAngle angles;
	angles.x = random->RandomFloat(0, 360);
	angles.y = random->RandomFloat(0, 360);
	angles.z = 0.0f;
	AngleVectors(angles, &vecVelocity);

	vecVelocity *= random->RandomFloat(200, 400);
	vecVelocity += pWeapon->GetAbsVelocity();
