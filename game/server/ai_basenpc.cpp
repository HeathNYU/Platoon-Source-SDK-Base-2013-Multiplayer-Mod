...
ConVar	sk_npc_stomach( "sk_npc_stomach","0.9" );
ConVar	sk_npc_arm( "sk_npc_arm","0.8" );
ConVar	sk_npc_leg( "sk_npc_leg","0.8" );
...
// Shoot trajectory
ConVar	ai_lead_time( "ai_lead_time","0.5" );
ConVar	ai_shot_stats( "ai_shot_stats", "0" );
ConVar	ai_shot_stats_term( "ai_shot_stats_term", "3000" );
...
bool CAI_BaseNPC::PreThink( void )
{
	...
	if ( (CAI_BaseNPC::m_nDebugBits & bits_debugDisableAI || !g_pAINetworkManager->NetworksLoaded()) )
	{
		if ( gpGlobals->curtime >= g_AINextDisabledMessageTime && !IsInCommentaryMode() )
		{
			...
			UTIL_HudMessageAll( tTextParam, "AI Disabled. \n" );
		}
...
void CAI_BaseNPC::NPCThink( void )
{
	...
	if ( m_bUsingStandardThinkTime )
	{
		...
		static float g_DecisionIntervals[] = 
		{
			.025,	// AIE_NORMAL
			.05,	// AIE_EFFICIENT
			.1,		// AIE_VERY_EFFICIENT
			.2,		// AIE_SUPER_EFFICIENT
		};
		...
		{
			SetNextThink( gpGlobals->curtime + .025 );
		}
		else
		{
...
void CAI_BaseNPC::StartNPC( void )
{
	...
	{
		.0, .150, .075, .225, .030, .180, .120, .270, .045, .210, .105, .255, .015, .165, .090, .240, .135, .060, .195, .215
	};
