...
ConVar hl2_sprintspeed( "hl2_sprintspeed", "260" );
...
#ifdef HL2_EPISODIC
	CSuitPowerDevice SuitDeviceFlashlight( bits_SUIT_DEVICE_FLASHLIGHT, 1.111 );	// 100 units in 90 seconds
#else
	CSuitPowerDevice SuitDeviceFlashlight( bits_SUIT_DEVICE_FLASHLIGHT, 2.222 );	// 100 units in 45 seconds
#endif
...
void CHL2_Player::StartSprinting( void )
{
	...
	if (pWeapon && !(m_nButtons & IN_ATTACK) && (m_nButtons & IN_FORWARD || m_nButtons & IN_BACK || m_nButtons & IN_MOVELEFT || m_nButtons & IN_MOVERIGHT) && !FClassnameIs(pWeapon, "weapon_physcannon"))
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
#define FLASHLIGHT_RANGE	Square(800)
...
void CHL2_Player::PickupObject( CBaseEntity *pObject, bool bLimitMassAndSize )
{
	...
	{
		if ( CBasePlayer::CanPickupObject( pObject, 50, 256 ) == false )
			 return;
	}
...
void CHL2_Player::DropActiveWeapon( void )
{
	...
	EyeVectors( &VecForward, NULL, &VecUp );

	VecForward *= 450.0f;
	VecUp *= 250.0f;
	Vec = VecForward + VecUp;

	BaseClass::Weapon_Drop( GetActiveWeapon(), NULL, &Vec );
	//HandleThrowGrenade();
}
