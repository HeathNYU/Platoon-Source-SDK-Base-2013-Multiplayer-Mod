...
//-----------------------------------------------------------------------------
// Purpose: Translate base class activities into combat activites
//-----------------------------------------------------------------------------
Activity CNPC_Combine::NPC_TranslateActivity( Activity eNewActivity )
{
	...
	if ( eNewActivity == ACT_IDLE_RPG )
		return ACT_IDLE_SMG1;
	if ( eNewActivity == ACT_IDLE_RPG_RELAXED )
		return ACT_IDLE_SMG1_RELAXED;
	if ( eNewActivity == ACT_IDLE_ANGRY_RPG )
		return ACT_IDLE_ANGRY_SMG1;
	if ( eNewActivity == ACT_WALK_RPG )
		return ACT_WALK_RIFLE_STIMULATED;
	if ( eNewActivity == ACT_WALK_CROUCH_RPG )
		return ACT_WALK_CROUCH_RIFLE;
	if ( eNewActivity == ACT_RUN_RPG )
		return ACT_RUN_AIM_RIFLE;
	if ( eNewActivity == ACT_RUN_CROUCH_RPG )
		return ACT_RUN_CROUCH_RIFLE;
	if (eNewActivity == ACT_RANGE_ATTACK_RPG)
...
int CNPC_Combine::SelectCombatSchedule()
{
	...
	// no ammo
	// ---------------------
	if ( ( GetActiveWeapon() && !FClassnameIs(GetActiveWeapon(), "weapon_rpg") && ( HasCondition(COND_NO_PRIMARY_AMMO) || HasCondition(COND_LOW_PRIMARY_AMMO) ) ) && !HasCondition(COND_CAN_MELEE_ATTACK1) )
	{
...
int CNPC_Combine::SelectSchedule( void )
{
	...
	if ( m_NPCState != NPC_STATE_SCRIPT)
	{
		...
		{
			DevMsg("Combine soldier in select schedule but RPG is guiding?\n");
			pRPG->StopGuiding();
		}
...
int CNPC_Combine::TranslateSchedule( int scheduleType ) 
{
	...
	case SCHED_HIDE_AND_RELOAD:
		{
			...
			// Stand();
			// DesireStand();
			if( CanGrenadeEnemy() && OccupyStrategySlot( SQUAD_SLOT_GRENADE1 ) && random->RandomInt( 0, 100 ) < 50 )
			{
				// If I COULD throw a grenade and I need to reload, 50% chance I'll throw a grenade before I hide to reload.
	...
	case SCHED_RANGE_ATTACK1:
		{
			if ( GetActiveWeapon() && !FClassnameIs(GetActiveWeapon(), "weapon_rpg") && ( HasCondition( COND_NO_PRIMARY_AMMO ) || HasCondition(COND_LOW_PRIMARY_AMMO) ) )
			{
			...
			if (GetActiveWeapon() && FClassnameIs(GetActiveWeapon(), "weapon_rpg"))
			{
				...
				{
					return SCHED_COMBINE_STRIDER_RANGE_ATTACK1_RPG;
				}
				else if (GetEnemy())
				{
					CBasePlayer *pPlayer = UTIL_GetNearestPlayer(GetEnemy()->GetAbsOrigin());
			...
			//Msg("Combine soldier using normal schedule.\n");
...
bool CNPC_Combine::CanAltFireEnemy( bool bUseFreeKnowledge )
{
	...
	if ( !(GetActiveWeapon() && GetActiveWeapon()->m_iClassname == FindPooledString("weapon_ar2")) )
		return false;
...
(
 ...
 "		TASK_RANGE_ATTACK1			0"
 "		TASK_COMBINE_IGNORE_ATTACKS	0.5"
 "		TASK_COMBINE_RPG_AUGER		1"
 ""
 "	Interrupts"
 )

 //=========================================================
 // > SCHED_COMBINE_STRIDER_RANGE_ATTACK1_RPG
 //=========================================================
 DEFINE_SCHEDULE
 (
 SCHED_COMBINE_STRIDER_RANGE_ATTACK1_RPG,
