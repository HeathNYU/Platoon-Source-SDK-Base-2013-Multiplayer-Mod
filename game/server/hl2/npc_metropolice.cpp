...
Activity CNPC_MetroPolice::NPC_TranslateActivity( Activity newActivity )
{
	...
	if (newActivity == ACT_IDLE_RPG)
		return ACT_IDLE_SMG1;
	if (newActivity == ACT_IDLE_RPG_RELAXED)
		return ACT_IDLE_SMG1_RELAXED;
	if (newActivity == ACT_IDLE_ANGRY_RPG)
		return ACT_IDLE_ANGRY_SMG1;
	if (newActivity == ACT_WALK_RPG)
		return ACT_IDLE_SMG1_STIMULATED;
	if (newActivity == ACT_RUN_RPG)
		return ACT_RUN_AIM_RIFLE;
	if (newActivity == ACT_RANGE_ATTACK_AR2)
		return ACT_RANGE_ATTACK_SMG1;
	if (newActivity == ACT_RANGE_ATTACK_RPG)
		return ACT_RANGE_ATTACK_SMG1;
...
int CNPC_MetroPolice::SelectSchedule( void )
{
	...
	{
		if ( GetActiveWeapon() && !FClassnameIs(GetActiveWeapon(), "weapon_rpg") && (GetActiveWeapon()->m_iClip1 <= 5) )
		{
			m_Sentences.Speak( "METROPOLICE_COVER_LOW_AMMO" );
			return SCHED_HIDE_AND_RELOAD;
		}
