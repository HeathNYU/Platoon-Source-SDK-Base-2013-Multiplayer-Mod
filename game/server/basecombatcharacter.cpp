...
//-----------------------------------------------------------------------------
void CBaseCombatCharacter::Weapon_Drop( CBaseCombatWeapon *pWeapon, const Vector *pvecTarget, const Vector *pVelocity)
{
	if ( !pWeapon )
		return;
	...
	*/
	...
	else
	{
		...
		{
			// Nowhere in particular; just drop it.
			float throwForce = ( IsPlayer() ) ? 500.0f : random->RandomInt( 64, 256 );
	...
	{
		if (GetAmmoCount(pWeapon->GetPrimaryAmmoType()) > 1)
		{
			CBaseCombatWeapon *pFrag = (CBaseCombatWeapon *)CBaseEntity::Create("weapon_frag", vecThrow, vec3_angle, NULL);
			Frag_Drop(pFrag, NULL, pVelocity);
		}
		else
		{
			pWeapon->Drop(vecThrow);
...
void CBaseCombatCharacter::Frag_Drop(CBaseCombatWeapon *pWeapon, const Vector *pvecTarget, const Vector *pVelocity)
{
	if (!pWeapon)
		return;
...
void CBaseCombatCharacter::Weapon_Equip( CBaseCombatWeapon *pWeapon )
{
	...
	if (pWeapon->GetMaxClip1() == -1)
	{
		...
		if( FStrEq(STRING(gpGlobals->mapname), "d3_c17_09") && FClassnameIs(pWeapon, "weapon_rpg") && pWeapon->NameMatches("player_spawn_items") )
		{
			...
			// start and the map is way too easy if you start with 3 RPG rounds. It's fine if a player conserves
