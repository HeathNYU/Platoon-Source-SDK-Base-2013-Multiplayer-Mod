...
void CBaseCombatWeapon::DropNew(const Vector &vecVelocity)
{
#if !defined( CLIENT_DLL )
...
bool CBaseCombatWeapon::ShouldDisplayReloadHUDHint()
{
	...
	if( pOwner != NULL && pOwner->IsPlayer() && UsesClipsForAmmo1() && m_iClip1 < (GetMaxClip1() / 3) )
	{
		// I'm owned by a player, I use clips, I have less then a third of a clip loaded. Now, does the player have more ammo?
		if ( pOwner )
		{
...
void CBaseCombatWeapon::SetPickupTouch( void )
{
	...
	if ( gpGlobals->maxClients > 0 )
	{
		if ( GetSpawnFlags() & SF_NORESPAWN )
		{
			...
			SetNextThink( gpGlobals->curtime + 60.0f + random->RandomFloat( 0.0f, 15.0f ) );
		}
	}
#endif
}
...
bool CBaseCombatWeapon::ReloadOrSwitchWeapons( void )
{
	...
	if ( !UsesClipsForAmmo1() && !HasAnyAmmo() && m_flNextPrimaryAttack < gpGlobals->curtime && m_flNextSecondaryAttack < gpGlobals->curtime )
	{
...
void CBaseCombatWeapon::ProcessAnimationEvents(void)
{
	...
	if (m_bInReload && pOwner->GetPlayerMaxSpeed() > 160 && (pOwner->m_nButtons & IN_FORWARD || pOwner->m_nButtons & IN_BACK || pOwner->m_nButtons & IN_MOVELEFT || pOwner->m_nButtons & IN_MOVERIGHT))
	{
		AbortReload();
		m_flNextPrimaryAttack = gpGlobals->curtime;
	}
...
void CBaseCombatWeapon::ItemPostFrame(void)
{
	...
	if ( !bFired && (pOwner->m_nButtons & IN_ATTACK) && (m_flNextPrimaryAttack <= gpGlobals->curtime) && !m_bWeaponIsLowered )
	{
		...
		else if (!UsesClipsForAmmo1() && pOwner->GetAmmoCount(m_iPrimaryAmmoType) <= 0)
		{
		...
		else
		{
			...
			if ( !m_bWeaponIsLowered && (( pOwner->m_afButtonPressed & IN_ATTACK ) || ( pOwner->m_afButtonReleased & IN_ATTACK2 )) )
			{
				m_flNextPrimaryAttack = gpGlobals->curtime;
			}
			PrimaryAttack();
...
{
	static Vector cone = VECTOR_CONE_9DEGREES;
	return cone;
}
...
bool CBaseCombatWeapon::DefaultReload( int iClipSize1, int iClipSize2, int iActivity )
{
	...
	m_flNextPrimaryAttack = m_flNextSecondaryAttack = m_flNextClipFill = flSequenceEndTime;
...
void CBaseCombatWeapon::WeaponIdle( void )
{
	...
	if ( HasWeaponIdleTimeElapsed() && !m_bWeaponIsLowered)
	{
...
void CBaseCombatWeapon::CheckReload( void )
{
	if ( m_bReloadsSingly )
	{
		...
		if ( (m_bInReload) && (m_flNextClipFill <= gpGlobals->curtime) && !m_bReloadCanceled )
		{
...
BEGIN_PREDICTION_DATA( CBaseCombatWeapon )
	...
	DEFINE_PRED_FIELD( m_bWeaponIsLowered, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE ),
...
BEGIN_NETWORK_TABLE_NOBASE( CBaseCombatWeapon, DT_LocalActiveWeaponData )
	...
	SendPropBool( SENDINFO( m_bWeaponIsLowered ) ),
	...
	RecvPropBool( RECVINFO( m_bWeaponIsLowered ) ),
#endif
END_NETWORK_TABLE()
