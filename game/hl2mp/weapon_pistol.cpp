...
{
	Vector vecShootOrigin;		//The origin of the shot
	QAngle	angShootDir;		//The angle of the shot
	...
	//DispatchParticleEffect("muzzle_smgs", vecShootOrigin, angShootDir);
...
void CWeaponPistol::ItemPostFrame( void )
{
	...
	if (!(pPlayer->m_nButtons & IN_ATTACK) && !m_bWeaponIsLowered && (m_flSoonestPrimaryAttack < gpGlobals->curtime))
	{
	...
	//else if ( ( pPlayer->m_nButtons & IN_ATTACK ) && ( m_flNextPrimaryAttack < gpGlobals->curtime ) && ( m_iClip1 <= 0 ) )
	//{
		//DryFire();
	//}
}
