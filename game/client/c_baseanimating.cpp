...
ConVar g_ragdoll_fadespeed( "g_ragdoll_fadespeed", "200" );
ConVar g_ragdoll_lvfadespeed( "g_ragdoll_lvfadespeed", "200" );
...
void C_BaseAnimating::FireEvent( const Vector& origin, const QAngle& angles, int event, const char *options )
{
	...
	case CL_EVENT_EJECTBRASS1:
		if ( m_Attachments.Count() > 0 && gpGlobals->curtime - m_flLastEjectTime >= 0.05 )
		{
				Vector attachOrigin;
				QAngle attachAngles;
				...
				if( GetAttachment( 2, attachOrigin, attachAngles ) )
				{
					...
					m_flLastEjectTime = gpGlobals->curtime;
				}
		}
		break;
