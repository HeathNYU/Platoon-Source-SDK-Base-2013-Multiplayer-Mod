...
#define OLD_ARMOR_BONUS  1.0	// Each Point of Armor is worth 1/x points of health

// New values
#define ARMOR_RATIO	0.3
...
bool CBasePlayer::ShouldTakeDamageInCommentaryMode( const CTakeDamageInfo &inputInfo )
{
	...
	// In commentary, ignore all damage except for bullets, buckshots, falling, and leeches
	if (!(inputInfo.GetDamageType() & (DMG_BULLET | DMG_BUCKSHOT | DMG_BURN | DMG_PLASMA | DMG_FALL | DMG_CRUSH)) && inputInfo.GetDamageType() != DMG_GENERIC)
		return false;
...
int CBasePlayer::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	if ( ( info.GetDamageType() & DMG_BUCKSHOT ) && g_pGameRules->IsMultiplayer() )
	{
		// buckshots damage armor more.
		flBonus *= 0.6;
	}
	...
	// Armor. 
	if (m_ArmorValue && !(info.GetDamageType() & (DMG_BULLET | DMG_FALL | DMG_DROWN | DMG_POISON | DMG_RADIATION)) )// armor doesn't protect against bullets, fall, or drown damage!
	{
		float temp = m_ArmorValue;

		float flNew = info.GetDamage() * (flRatio + (0.8 - flRatio) * (1 - (temp / 100)));
...
void CBasePlayer::Weapon_Drop( CBaseCombatWeapon *pWeapon, const Vector *pvecTarget /* = NULL */, const Vector *pVelocity /* = NULL */ )
{
	...
	{
		if (FClassnameIs(pWeapon, "weapon_frag") && GetAmmoCount(pWeapon->GetPrimaryAmmoType()) > 1)
		{
			bWasActiveWeapon = false;
