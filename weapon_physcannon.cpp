...
ConVar physcannon_maxmass( "physcannon_maxmass", "300", FCVAR_REPLICATED | FCVAR_CHEAT );
ConVar physcannon_tracelength( "physcannon_tracelength", "300", FCVAR_REPLICATED | FCVAR_CHEAT );
ConVar physcannon_chargetime("physcannon_chargetime", "2", FCVAR_REPLICATED | FCVAR_CHEAT );
ConVar physcannon_pullforce( "physcannon_pullforce", "5000", FCVAR_REPLICATED | FCVAR_CHEAT );
...
ConVar player_throwforce( "player_throwforce", "600", FCVAR_REPLICATED | FCVAR_CHEAT );
...
void CPlayerPickupController::Shutdown( bool bThrown )
{
	...
	if ( m_pPlayer )
	{
		...
		if ( pWeapon )
		{
			m_pPlayer->Weapon_Switch(pWeapon);
		}
		else
...
void CPlayerPickupController::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( ToBasePlayer(pActivator) == m_pPlayer )
	{
		...
		if ( m_pPlayer->m_nButtons & IN_ATTACK )
		{
			...
			float massFactor = clamp( pPhys->GetMass(), 0.5, 50 );
			massFactor = RemapVal( massFactor, 0.5, 50, 0.5, 50 );
...
void CWeaponPhysCannon::PuntVPhysics( CBaseEntity *pEntity, const Vector &vecForward, trace_t &tr )
{
	...
	if ( Pickup_OnAttemptPhysGunPickup( pEntity, pOwner, PUNTED_BY_CANNON ) )
	{
		...
		if ( !Pickup_ShouldPuntUseLaunchForces( pEntity, PHYSGUN_FORCE_PUNTED ) )
		{
			...
			float mass = MIN(totalMass, maxMass); // max 300kg of additional force
