...
LINK_ENTITY_TO_CLASS( platoon_gamerules, CHL2MPGameRulesProxy );
...
static const char *s_PreserveEnts[] =
{
	...
	BEGIN_RECV_TABLE( CHL2MPGameRulesProxy, DT_HL2MPGameRulesProxy )
		RecvPropDataTable( "platoon_gamerules_data", 0, 0, &REFERENCE_RECV_TABLE( DT_HL2MPRules ), RecvProxy_HL2MPRules )
	END_RECV_TABLE()
	...
	BEGIN_SEND_TABLE( CHL2MPGameRulesProxy, DT_HL2MPGameRulesProxy )
		SendPropDataTable( "platoon_gamerules_data", 0, &REFERENCE_SEND_TABLE( DT_HL2MPRules ), SendProxy_HL2MPRules )
	END_SEND_TABLE()
#endif
...
CHL2MPRules::CHL2MPRules()
{
	...
	CreateEntityByName("platoon_gamerules");
...
void CHL2MPRules::ExtendRoundtimer(int iExtendedTime)
{
	m_iDuration += iExtendedTime;
}
#endif
...
void CHL2MPRules::CreateStandardEntities( void )
{
	...
	CBaseEntity *pEnt = 
#endif
	CBaseEntity::Create( "platoon_gamerules", vec3_origin, vec3_angle );
	Assert( pEnt );
#endif
}
...
void CHL2MPRules::Think( void )
{
	...
	if (m_PrepStage && m_iRemain != GetRoundtimerRemain())
	{
		m_iRemain = GetRoundtimerRemain();
		char RoundtimerRemain[64];
		Q_snprintf(RoundtimerRemain, sizeof(RoundtimerRemain), "%d", GetRoundtimerRemain());

		UTIL_ClientPrintAll( HUD_PRINTCENTER, "Welcome to the jungle, soldier. Round starts in %s1 %s2", RoundtimerRemain, GetRoundtimerRemain() == 1 ? "second." : "seconds." );
		...
		if (m_iRemain == 11)
		{
			...
			tTextParam.holdTime = 3;
			tTextParam.fxTime = 0;
			tTextParam.channel = 2;
			UTIL_HudMessageAll(tTextParam, "Platoon: Checkpoint Mode\n");
		}
		...
		if (m_iRemain == 6)
		{
			...
			tTextParam.fxTime = 0;
			tTextParam.channel = 2;
			UTIL_HudMessageAll(tTextParam, "A lovely mod created by @HeathNYU\nat github.com\n");
		}
	}
	else if (!m_PrepStage && m_GameStage < 3)
	{
		hudtextparms_s tTextParam;
		tTextParam.x = 0.65;
		tTextParam.y = 0.55;
		...
		if (m_GameStage == 0)
		{
			UTIL_HudMessageAll(tTextParam, "Advance through NVA resistance\nand eliminate all radio stations.\n");
			m_iRemain = GetRoundtimerRemain();
			m_GameStage++;
		}
		else if (m_iRemain - GetRoundtimerRemain() > 4)
		{
			m_iRemain = GetRoundtimerRemain();
			if (m_GameStage == 1)
			{
				UTIL_HudMessageAll(tTextParam, "Radio stations provide NVA\nreinforcement.\n");
				m_GameStage++;
			}
			else if (m_GameStage == 2)
			{
				UTIL_HudMessageAll(tTextParam, "Good luck! \n");
				m_GameStage++;
			}
		}
	}
...
//	float flTimeLimit = mp_timelimit.GetFloat() * 60;
	...
	if (m_bAwaitingReadyRestart && m_bHeardAllPlayersReady)
	{
		...
		m_flRestartGameTime = gpGlobals->curtime + 5.0;
		m_bAwaitingReadyRestart = false;
	}
...
//-----------------------------------------------------------------------------
// Purpose: Lost conditions
//-----------------------------------------------------------------------------

	if ( GetRoundtimerRemain() <= 0)
	{
		if (m_PrepStage)
		{
			if (gpGlobals->curtime > m_tmNextPeriodicThink)
			{
				...
				InstantCheckRestartGame();
				m_tmNextPeriodicThink = gpGlobals->curtime + 2.0;
			}
		...
		else
		{
			if (gpGlobals->curtime > m_tmNextPeriodicThink)
			{
				//Do "We'll get 'em next time" effects
				CheckAllPlayersReady();
				DelayedCheckRestartGame();
				m_tmNextPeriodicThink = gpGlobals->curtime + 11.0;
			}
	...
	if ( NumOfPlayersAlive() <= 0)
	{
		if (gpGlobals->curtime > m_tmNextPeriodicThink)
		{
			...
			CheckAllPlayersReady();
			DelayedCheckRestartGame();
			m_tmNextPeriodicThink = gpGlobals->curtime + 6.0;
		}
...
CAmmoDef *GetAmmoDef()
{
	...
	if (!bInitted)
	{
		...
		def.AddAmmoType("AR2", DMG_BULLET, TRACER_LINE_AND_WHIZ, "sk_plr_dmg_ar2", "sk_npc_dmg_ar2", "sk_max_ar2", BULLET_IMPULSE(800, 3000), 0);
		def.AddAmmoType("AlyxGun", DMG_BULLET, TRACER_LINE, "sk_plr_dmg_alyxgun", "sk_npc_dmg_alyxgun", "sk_max_alyxgun", BULLET_IMPULSE(800, 2000), 0);
		def.AddAmmoType("Pistol", DMG_BULLET, TRACER_LINE_AND_WHIZ, "sk_plr_dmg_pistol", "sk_npc_dmg_pistol", "sk_max_pistol", BULLET_IMPULSE(800, 2000), 0);
		def.AddAmmoType("SMG1", DMG_BULLET, TRACER_LINE_AND_WHIZ, "sk_plr_dmg_smg1", "sk_npc_dmg_smg1", "sk_max_smg1", BULLET_IMPULSE(800, 2500), 0);
		def.AddAmmoType("357", DMG_BULLET, TRACER_LINE_AND_WHIZ, "sk_plr_dmg_357", "sk_npc_dmg_357", "sk_max_357", BULLET_IMPULSE(800, 3000), 0);
		def.AddAmmoType("XBowBolt", DMG_BULLET, TRACER_LINE, "sk_plr_dmg_crossbow", "sk_npc_dmg_crossbow", "sk_max_crossbow", BULLET_IMPULSE(800, 5000), 0);

		def.AddAmmoType("Buckshot", DMG_BUCKSHOT, TRACER_LINE, "sk_plr_dmg_buckshot", "sk_npc_dmg_buckshot", "sk_max_buckshot", BULLET_IMPULSE(600, 1500), 0);
		def.AddAmmoType("RPG_Round", DMG_BURN, TRACER_NONE, "sk_plr_dmg_rpg_round", "sk_npc_dmg_rpg_round", "sk_max_rpg_round", 0, 0);
		def.AddAmmoType("SMG1_Grenade", DMG_BURN, TRACER_NONE, "sk_plr_dmg_smg1_grenade", "sk_npc_dmg_smg1_grenade", "sk_max_smg1_grenade", 0, 0);
		def.AddAmmoType("SniperRound", DMG_BULLET | DMG_SNIPER, TRACER_NONE, "sk_plr_dmg_sniper_round", "sk_npc_dmg_sniper_round", "sk_max_sniper_round", BULLET_IMPULSE(800, 6000), 0);
		def.AddAmmoType("SniperPenetratedRound", DMG_BULLET | DMG_SNIPER, TRACER_NONE, "sk_dmg_sniper_penetrate_plr", "sk_dmg_sniper_penetrate_npc", "sk_max_sniper_round", BULLET_IMPULSE(400, 6000), 0);
		...
		def.AddAmmoType("AirboatGun", DMG_AIRBOAT, TRACER_LINE, "sk_plr_dmg_airboat", "sk_npc_dmg_airboat", NULL, BULLET_IMPULSE(400, 6000), 0);
		...
		def.AddAmmoType("HelicopterGun", DMG_BULLET, TRACER_LINE_AND_WHIZ, "sk_npc_dmg_helicopter_to_plr", "sk_npc_dmg_helicopter", "sk_max_smg1", BULLET_IMPULSE(800, 5000), AMMO_FORCE_DROP_IF_CARRIED | AMMO_INTERPRET_PLRDAMAGE_AS_DAMAGE_TO_PLAYER);
...
void CHL2MPRules::InstantCheckRestartGame( void )
{
	...
	if( mp_readyrestart.GetBool() )
	{
		...
		mp_readyrestart.SetValue( iRestartDelay );
...
void CHL2MPRules::CheckRestartGame( void )
{
	...
	if ( iRestartDelay > 0 )
	{
		...
		UTIL_ClientPrintAll( HUD_PRINTCENTER, "Attention! Game will restart in %s1 %s2", strRestartDelay, iRestartDelay < 2 ? "second." : "seconds." );
		UTIL_ClientPrintAll( HUD_PRINTCONSOLE, "Attention! Game will restart in %s1 %s2", strRestartDelay, iRestartDelay < 2 ? "second." : "seconds." );
...
void CHL2MPRules::DelayedCheckRestartGame( void )
{
	// Restart the game if specified by the server
	int iRestartDelay = 10;
	...
	if ( iRestartDelay > 0 )
	{
		...
		tTextParam.b1 = 144;
		tTextParam.a1 = 255;
		tTextParam.fadeinTime = 0.05;
		tTextParam.fadeoutTime = 0.5;
		tTextParam.holdTime = 6;
		tTextParam.fxTime = 0;
		tTextParam.channel = 1;

		UTIL_HudMessageAll(tTextParam, "American forces have been overrun. NVA is victorious!\nPresident Johnson is renewing the offensive. Please stand by.\n");

		m_flRestartGameTime = gpGlobals->curtime + iRestartDelay;
		m_bCompleteReset = true;
		mp_restartgame.SetValue( iRestartDelay );
	}
...
void CHL2MPRules::InitDefaultAIRelationships(void)
{
	...
	*/
	...
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_COMBINE, CLASS_PLAYER, D_NU, 0);
	...
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_COMBINE, CLASS_CITIZEN_REBEL, D_NU, 0);
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_COMBINE, CLASS_COMBINE, D_NU, 0);
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_COMBINE, CLASS_COMBINE_GUNSHIP, D_NU, 0);
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_COMBINE, CLASS_COMBINE_HUNTER, D_HT, 0);
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_COMBINE, CLASS_CONSCRIPT, D_NU, 0);
	...
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_COMBINE, CLASS_MANHACK, D_HT, 0);
	...
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_COMBINE, CLASS_SCANNER, D_HT, 0);
	...
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_COMBINE, CLASS_PLAYER_ALLY, D_NU, 0);
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_COMBINE, CLASS_PLAYER_ALLY_VITAL, D_NU, 0);
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_COMBINE, CLASS_HACKED_ROLLERMINE, D_NU, 0);
	...
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_COMBINE_GUNSHIP, CLASS_METROPOLICE, D_HT, 0);
	...
	*/
	...
	CBaseCombatCharacter::SetDefaultRelationship(CLASS_METROPOLICE, CLASS_COMBINE_GUNSHIP, D_FR, 0);
